...
const { smsGatewayUnitel, smsGatewayApiCom } = require('../smsGateways')

class notificationSMSServiceV2 {
	db = null
	message = null
	...
	processCondition(data, eventRule) {
		Object.entries(eventRule.condition).map(([key, value]) => {
			if (this.message !== null) {
				...
				this.message = this.message.replace(`@DATA_${key.toUpperCase()}@`, data.message[value])
			}
		})
	}
	async processStatic(data, eventRule) {
		if (this.message !== null) {
			...
			this.message = this.message.replace(`@DEVICE_NAME@`, data.device.name)
	...
	async sendNotification(eventAction) {
		...
		switch (eventAction.config.gateway) {
			case 'uni-tel':
				...
				break;
			case 'gatewayapi.com':
				smsGateway = new smsGatewayApiCom(this.db, eventAction.host)
				break;
...
module.exports = notificationSMSServiceV2