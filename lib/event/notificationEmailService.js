...
class notificationEmailService {
	...
	async execute(data, eventRule, eventAction) {
		...
		this.sendNotificartion(eventAction, eventRule)
	}
	...
	async sendNotificartion(eventAction, eventRule) {
		const mailService = new sentiMail(process.env.SENDGRID_API_KEY, this.db)

		eventAction.config.recipients.map(async recipient => {
			let msg = await mailService.getMailMessageFromTemplateType(5, { "@NAME@": recipient.name, "@MESSAGE@": this.mailBody }, eventRule.host) // JSON.stringify(this.mailBody)
			if (this.subject !== null) {
