...
const sentiDeviceLocationUpdate = require('./sentiDeviceLocationUpdate')
...
const sentiEventRuleRunning = require('../../server').sentiEventRuleRunning
...
class eventServiceV2 {
	...
	async findRulesAndExecute(data, deviceTypeId = null, registryId = null, deviceId = null) {
		...
		if (rs[0].length > 0) {
			rs[0].forEach(async (row) => {
				...
				let runningKey = eventRule.uuid + '::' + eventRule.deviceTypeUUID + '::' + eventRule.registryUUID + '::' + eventRule.deviceUUID
				if (eventRule.config.notificationStrategy !== eventServiceV2.notificationStrategy.always_notrack) {
					if (sentiEventRuleRunning.startRunning(runningKey) === true) {
						return
					}
				}
				let condition = await this.testRule(data, eventRule)
				let event = await this.shouldExecute(data, eventRule, condition.state)
				// console.log(data, eventRule, event, condition)
				if (condition.state !== null && event.shouldExecute !== false) {
					//console.log('execute', event)
					data.sentiCondition = condition
					await this.findActionsAndExecute(data, eventRule, event.state)
				} else {
					// console.log('no execute', condition, eventRule.condition, data)
				}
				if (eventRule.config.notificationStrategy !== eventServiceV2.notificationStrategy.always_notrack) {
					sentiEventRuleRunning.endRunning(runningKey)
				}
			})
		}
	}
	...
	async shouldExecute(data, eventRule, conditionState) {
		...
		if (eventRule.config.notificationStrategy === eventServiceV2.notificationStrategy.always_notrack) {
			event.state = Event.states.active
	...
	async executeAction(data, eventRule, eventAction) {
		switch (eventAction.type) {
			...
			case eventServiceV2.actionTypes.mqttPublish:
				...
				break
			case eventServiceV2.actionTypes.deviceLocationUpdate:
				const deviceLocationUpdate = new sentiDeviceLocationUpdate(mysqlConn)
				deviceLocationUpdate.execute(data, eventRule, eventAction)
				break
	...
	testCondition(data, condition) {
		...
		if (data[condition.metric] === "SENTI_ALWAYS_TRUE") {
			return true
		}
	...
	async testCloudFunction(data, eventRule) {
		...
		let result = (typeof decodedTest.data === "boolean") ? { state: decodedTest.data } : (decodedTest.data !== null && typeof decodedTest.data.state === "object") ? decodedTest.data : { state: null }
		return result
	}
	/**
	 *
	...
	async testRule(data, eventRule) {
		...
		if (Array.isArray(eventRule.condition)) {
			return { state: this.testConditions(data.message, eventRule.condition) }
		}
		// V1 logical test
		return { state: this.testCondition(data.message, eventRule.condition) }
	}
