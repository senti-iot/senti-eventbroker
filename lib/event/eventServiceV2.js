...
class eventServiceV2 {
	...
	async findRulesAndExecute(data, deviceTypeId = null, registryId = null, deviceId = null) {
		...
		if (rs[0].length > 0) {
			rs[0].forEach(async (row) => {
                ...
                if (await this.shouldExecute(data, eventRule, conditionState) !== false) {
                    console.log('execute')
                    await this.findActionsAndExecute(data, eventRule, conditionState)
                } else {
    ...
    async shouldExecute(data, eventRule, conditionState) {
        ...
        if (rs[0].length === 1) {
            ...
            if(conditionState === true) {
                event = new Event({ uuid: uuidv4(), ruleUUID: eventRule.uuid, deviceUUID: eventRule.deviceUUID, state: 1, count: 1, created: data.messageMeta.cleanTime, modified: data.messageMeta.cleanTime })
	...
	async saveEvent(event) {
        ...
        console.log(event)
        if (event.id !== null) {
            let update = `UPDATE event SET uuid = ?, ruleUUID = ?, deviceUUID = ?, count = ?, state = ?, created = ?, modified = ?, lastAction = ?, nextAction = ? WHERE id = ?`
            console.log(await mysqlConn.format(update, [event.uuid, event.ruleUUID, event.deviceUUID, event.count, event.state, event.created, event.modified, event.lastAction, event.nextAction, event.id]))
            await mysqlConn.query(update, [event.uuid, event.ruleUUID, event.deviceUUID, event.count, event.state, event.created, event.modified, event.lastAction, event.nextAction, event.id])
        } else {
	 ...
	 */
	async findActionsAndExecute(data, eventRule, conditionState) {
		let select = `SELECT id, uuid, ruleUUID, type, config, conditionState, deleted, host FROM eventAction WHERE ruleUUID = ? AND conditionState = ? AND deleted = 0`
		conditionState = (conditionState === true) ? 1 : 0
		console.log(await mysqlConn.format(select, [eventRule.uuid, conditionState]))
		let rs = await mysqlConn.query(select, [eventRule.uuid, conditionState])
		if (rs[0].length > 0) {
			rs[0].forEach(async (row) => {
