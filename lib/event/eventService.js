			...
			if (insertQuery[0].insertId) {
				...
				if (fAlarm) {
					...
					let insActionSQL = `INSERT INTO eventAction
										...
										VALUES(?, ?, ?, 0, ?);`
					let insActionQuery = await this.db.query(insActionSQL, [fAlarm.id, a.notification.type, JSON.stringify(a.notification.config), a.host])
	...
	async getEventRulesByUUID(uuid) {
		if (uuid) {
			let selectSQL = `SELECT eR.*,
							JSON_ARRAYAGG(JSON_OBJECT('type', eA.\`type\`, 'config', eA.config)) as actions,
							e.count, e.state
							from eventRule eR
							INNER JOIN eventUser eU on eU.eventUUID = eR.uuid
							INNER JOIN eventAction eA on eA.ruleId = eR.id
							INNER JOIN event e on e.ruleId = eR.id
							where eU.userUUID=? and deleted=0`
