...
class eventService {
	...
	async createEventRule(a) {
		if (a) {
			let insertSQL = `INSERT INTO eventRule
							 (uuid, name, dataSource, condition, cloudFunction, ttl, config, deviceId, deleted, host)
							 VALUES(?,?,?,?,?,?,?,?,0,?);`
			let uuid = uuidv4()
			let insertQuery = await this.db.query(insertSQL, [uuid, a.name, a.dataSource, a.condition, a.cloudFunction, a.ttl, a.config, a.deviceId, a.host])
			let insertFormat = await this.db.format(insertSQL, [uuid, a.name, a.dataSource, a.condition, a.cloudFunction, a.ttl, a.config, a.deviceId, a.host])
			...
			if (insertQuery[0].insertId) {
				...
				if (fAlarm) {
					...
					let insActionSQL = `INSERT INTO eventAction
										...
										VALUES(?, ?, ?, 0, ?);`
					let insActionQuery = await this.db.query(insActionSQL, [fAlarm.id, a.notification.type, a.notification.config, a.host])
					if (insActionQuery[0].insertId) {
						/**
	...
	async getEventRuleByUUID(uuid) {
		if (uuid) {
			let selectSQL = `SELECT * from eventRule where uuid=? and deleted=0`
