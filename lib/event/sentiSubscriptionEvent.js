		...
		if(subscriptionRules !== false) {
			await subscriptionRules.reduce(async (promise, subscriptionRule) => {
				...
				try {
					...
					this.logFunction(`Test result for cloud function ${subscriptionRule.cloudFunction} for subscription UUID: ${this.uuid}: ${JSON.stringify(testResult)}`, Object.keys(testResult.test))
					if(testResult.state === true) {
						console.log(subscriptionRule.config)
						subscriptionRule.config.actionConfig.message.body = testResult.message || ''
