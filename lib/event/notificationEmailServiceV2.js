...
class notificationEmailServiceV2 {
	...
	processCondition(data, eventRule) {
		Object.entries(eventRule.condition).map(([key, value]) => {
			if (this.subject !== null) {
				...
				this.subject = this.subject.replace(`@DATA_${key.toUpperCase()}@`, data.message[value])
			}
			if (this.mailBody !== null) {
				this.mailBody = this.mailBody.replace(`@${key.toUpperCase()}@`, value)
				this.mailBody = this.mailBody.replace(`@DATA_${key.toUpperCase()}@`, data.message[value])
			}
		})
	}
	async processStatic(data, eventRule) {
		if (this.subject !== null) {
			...
			this.subject = this.subject.replace(`@DEVICE_NAME@`, data.device.name)
		}
		if (this.mailBody !== null) {
			this.mailBody = this.mailBody.replace(`@EVENT_ID@`, eventRule.id + '-' + eventRule.eventId)
			this.mailBody = this.mailBody.replace(`@DEVICE_NAME@`, data.device.name)
