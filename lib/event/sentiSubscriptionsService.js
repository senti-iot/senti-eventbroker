...
class sentiSubscriptionService {
	...
	async deleteSubscriptionRule(id = false) {
		if (id === false) {
			return false
		}
		let del = `DELETE FROM eventSubscriptionRule WHERE id = ?`
		console.log(mysqlConn.format(del, [id]))
		let rs = await mysqlConn.query(del, [id]).catch(err => {
			console.log(err)
		})
	...
	async getSubscriptionRuleByUuid(uuid = false) {
		if (uuid === false) {
			return false
		}
		let select = `SELECT e.id, e.uuid, e.subscriptionId, e.parentOrg, e.parentUser, e.cloudFunction, e.condition, e.config
	...
	async updateSubscriptionRule(data) {
		...
		if (rs[0].affectedRows === 1) {
			return await this.getSubscriptionRuleById(data.id)
		}
		return false
	}
